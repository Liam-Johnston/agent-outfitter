...
describe("loadManifest", () => {
  test("loads and validates the YAML form from the spec", async () => {
    ...
    await writeFileAt(root, "skills.config.yaml", YAML_MANIFEST);

    const { manifest, path, writable } = await loadManifest({ root });
    expect(path).toBe(join(root, "skills.config.yaml"));
  ...
  test("returns an empty manifest when none exists", async () => {
    ...
    expect(manifest).toEqual({ version: 1, sources: [], mcp: [] });
  });
  ...
  test("loads a TypeScript config and treats it as read-only", async () => {
    ...
    expect(path).toBe(join(root, "skills.config.ts"));
    expect(writable).toBe(false);
    expect(manifest.sources).toEqual(["local:./skills"]);
  });

  test("prefers skills.config.ts over the YAML form", async () => {
    const root = await makeTempDir();
    await writeFileAt(root, "skills.config.yaml", "version: 1\n");
    await writeFileAt(root, "skills.config.ts", "export default { version: 1 };\n");
    expect(await findManifest(root)).toBe(join(root, "skills.config.ts"));
  });

  test("rejects an unknown version and unknown keys", async () => {
    const root = await makeTempDir();
    await writeFileAt(root, "skills.config.yaml", "version: 2\n");
    await expect(loadManifest({ root })).rejects.toThrow(ManifestError);

    const other = await makeTempDir();
    await writeFileAt(other, "skills.config.yaml", "version: 1\nsauces: []\n");
...
describe("writeManifest", () => {
  test("round-trips YAML", async () => {
    const root = await makeTempDir();
    const path = join(root, "skills.config.yaml");
  ...
  test("drops live target adapters, keeping named ones", async () => {
    const root = await makeTempDir();
    const path = join(root, "skills.config.json");
    await writeManifest(path, {
      version: 1,
      targets: ["codex", { name: "custom", resolveSkillsDir: () => "", materialize: async () => ({ path: "" }) }],
      sources: [],
    });
  ...
  test("refuses to write a TypeScript manifest", async () => {
    const root = await makeTempDir();
    await expect(writeManifest(join(root, "skills.config.ts"), { version: 1 })).rejects.toThrow(
      ManifestError,
    );
  });
});
